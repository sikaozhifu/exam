...
public class MD5Util {
    ...
    /* 是最新一次计算结果的2进制内部表示，表示128bit的MD5值 */
    private byte[] digest = new byte[16];

    /**
     * 主要的公共方法，入口参数是你想要进行MD5变换的字符串
    ...
    public String getMD5OfBytes(byte[] byteBuffer) {
        md5Update(byteBuffer, byteBuffer.length);
        md5Final();
        digestHexStr = "";
        for (int i = 0; i < 16; i++) {
            digestHexStr += byteHEX(digest[i]);
    ...
    /**
     * MD5的主计算过程，inbuf是要变换的字节串，inputlen是长度，这个
    ...
    public static String toMD5(String source) {
        return md5Util.getMD5ofStr(source);
    }
